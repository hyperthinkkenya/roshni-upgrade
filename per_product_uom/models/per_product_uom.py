...
class local_product_uom(models.Model):
    _inherits = {'uom.uom':'uid', }
    _name = "localproduct.uom"
    _description = "Local Product Uom"



    uid = fields.Many2one('uom.uom', ondelete='cascade', index=True, required=True)
    ...
    #We need to delete the corresponding records in uom.uom. overriding unlink() lets us do that.
    def unlink(self):
        for x in range(len()):
...
class product_uom_class(models.Model):
    _inherits = {'uom.category':'catid'}
    _name = 'productuom.class'
    _description = "Product Uom Class"

    catid = fields.Many2one('uom.category', ondelete='cascade', index=True, required=True)
...
class ProductTemplate(models.Model):
    ...
    def onchange_uom_class(self):
        ...
        #Here we set the domain to only allow the selection of the proper per product uoms
            ...
            records = self.env['uom.uom'].search([('category_id.name','=',self.uom_class.name),('name','=',self.uom_id.name)],limit=1)
            if records:
                self.uom_id = records[0]
            else:
                self.uom_id = False

            records = self.env['uom.uom'].search([('category_id.name', '=', self.uom_class.name), ('name', '=', self.uom_po_id.name)], limit=1)
            if records:
